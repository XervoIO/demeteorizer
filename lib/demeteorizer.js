...
Demeteorizer.prototype.filterDep = function (name, version) {
  ...
  if (filtered && filtered.version) {
    filtered.version = version.replace(
      /registry.beneaththeink.com/,
      'registry.npmjs.com');
  }
...
Demeteorizer.prototype.createPackageJSON = function (context, callback) {
  ...
  // Merge any arbitrary JSON from the user.
  Object.keys(context.json).forEach(function (k) {
    packageJSON[k] = context.json[k];
  });
