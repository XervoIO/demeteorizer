...
//--------------------------------------------------------------------------------------------------
Demeteorizer.prototype.convert = function(input, output, nodeVersion, release, tarball, callback) {
    ...
    files.forEach(function(file) {
      ...
      if(stats.isDirectory()) {
        ...
        if(fs.existsSync(package)) {
          ...
          // Meteor is using http-proxy v1.0.0, which is not in npm.
          // Overwrite dependency to version that is in npm.
          if(contents.name === 'http-proxy') {
            dependencies[contents.name] = '0.10.3';
          }
        }
      }
    });
